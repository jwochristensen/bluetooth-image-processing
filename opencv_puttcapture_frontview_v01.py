...
vidfilename = '200613_Putt10ft_video.mov'
...
#automatically set a video path (for testing)
if not args.get("video", False):
    args["video"] = vidfilename
#print(args)
...
#area to focus movement detection - make dynamic based on resolution??
...
#crude conversions of starting points/ball pixel size to distance covered - need something better
putt_radius_dist_array = [[1,20],[2,15],[3,10]]
putt_ycoord_dist_array = [[0,200,4444],[200,250,25],[250,280,20],[280,330,15],[330,450,10],[450,600,5],[600,840,3]]
...
#convert putt data to distance estimate - VERY IMPORTANT!!! Improve at some point
...
def find_direction_and_trace_path(pts, frame, buffer, counter, puttid):
    ...
    dX = 0
    dY = 0
    len_puttcolors = len(putt_colors) #determine how many colors are available
    puttcolor_index = puttid % len_puttcolors - 1 #compare puttid to number of colors available and get index
    puttcolor = putt_colors[puttcolor_index] #get color corresponding to index
...
def update_puttlog(puttlog, puttid):
    ...
    if cnt_records > 10:
        ...
        #!!!!will need to check timestamps of holeview putt capture to link putt to result; perhaps setup holeview as a class obj to be initialized with puttlog?
...
def getCameraVideo():
    cam = VideoStream(src=0).start() #usb top (where you specify the source of video stream)
    time.sleep(2.0)
    return cam
...
res_initialize = True #set variable to get frame resolution on first loop
...
while True:
    ...
    #if testing, get a screen print of one of first frames
    if (counter < 2 and testing == True):
        img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        #plt.imshow(img)
        #plt.show()

    #get frame size on first loop:
    if res_initialize:
        #print('frame type: ', type(frame))
        #img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        #print('img type: ', type(img))
        #img_tuple = cv2.imread(img)
        print('frame width:  ', frame.shape[0])
        print('frame height: ', frame.shape[1])
        res_initialize = False

    try:
        mask_white = create_white_mask(frame) #create white mask to find white objects

        mask_backsub = create_background_mask(mask_white, backSub) #subract background - only look for changes
        #cv2.imshow("background mask", mask_backsub)

        mask_roi = create_region_ofinterest(mask_backsub, pts_focusArea) #only look for objects in region of interest
        ...
        #only look for specific types of contours - can add more filtering to this for a golf ball (below horizon, square-ish aspect ratio, surrounded by green?, has moved xy distance?)
        cnts = screen_contours(cnts, pts_history, xRange, yRange)
        ...
        if len(cnts) > 0:
            ...
            if radius > mincntradius:
                ...
                #check if center is significantly far from last point
